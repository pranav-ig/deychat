...
var MyContractJSON =require(path.join(__dirname, 'build/contracts/CertificateUpload.json'));
var web3 = new Web3("http://localhost:7545");

//-sss- Edit Start Here ---
coinbase = "0x1C9789aeD21e37219C8482dD6300E84d944cb8f2";
...
app.post("/uploadCertificate", function(req, res, next) {
...
ipfs.files.add(fileBytes, function (err, file) {
  ...
  SMS.methods.setCertificate(email, ifpsHash).send({ from: coinbase }).then(function(result){
  res.send(file);
  })
...
app.get('/viewCertificate', function(req, res, next){

  data = req.query;
  console.log(data);
  SMS.methods.getCertificate(data.email)
...
app.get("/ipfs"), function (req, res) {
  res.sendFile(path.join(public, "ipfs.html"));
}
